...
uint8_t crsf_buffer[CRSF_BUFF_CNT] = {0};
uint16_t crsf_channels[CRSF_CHAN_CNT] = {0};
...
void crsf_decode()
{
    ...
    if(crc == crsf_buffer[25]) {
        ...
        if (crsf_buffer[1] == 24) // this looks like a check against frame length, TODO: check
        {
...
//        }
        ...
        if (bi_direction) {
            if((crsf_channels[crsf_input_channel] > (CRSF_CHANNEL_VALUE_MID - 4)) && (crsf_channels[crsf_input_channel] < (CRSF_CHANNEL_VALUE_MID + 4))) {
                newinput = 1000;
                is_zero = 1;
            } else {
                newinput = map(crsf_channels[crsf_input_channel], CRSF_CHANNEL_VALUE_MIN, CRSF_CHANNEL_VALUE_MAX, 0, 2000);
            }
        } else {
            if (crsf_channels[crsf_input_channel] <= CRSF_CHANNEL_VALUE_MIN) {
                newinput = 0;
                is_zero = 1;
            } else {
                newinput = map(crsf_channels[crsf_input_channel], CRSF_CHANNEL_VALUE_MIN, CRSF_CHANNEL_VALUE_MAX, 47, 2047);
            }
        }
